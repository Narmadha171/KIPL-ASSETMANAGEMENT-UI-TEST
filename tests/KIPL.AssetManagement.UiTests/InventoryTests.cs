...
{
    [Test]
    public async Task ITManager_Verify_All_Features_In_Single_Session()
    {
        // 1. Log in once as IT Asset Manager
        ...
        // Verify Bulk import button is visible for IT Asset Manager
        ...
        string uniqueSerial = "SN-Manager-" + System.Guid.NewGuid().ToString().Substring(0, 8);
        await Page.SelectOptionAsync("#modal-addAsset select[name='Input.Category']", new[] { "0" }); // Laptop
        await Page.FillAsync("#modal-addAsset input[name='Input.Brand']", "Manager Brand");
        await Page.FillAsync("#modal-addAsset input[name='Input.Model']", "Manager Model");
        await Page.FillAsync("#modal-addAsset input[name='Input.SerialNumber']", uniqueSerial);
        await Page.FillAsync("#modal-addAsset textarea[name='Input.Notes']", "Added by Playwright IT Manager test.");
        ...
        var assetRow = Page.Locator("tbody tr.data-row:has-text('Manager Brand')").First;
        ...
        await Page.FillAsync("#modal-assignDirect input[name='notes']", "Onboarding Kit Manager");
        ...
        await Page.FillAsync("input[id='Search']", uniqueSerial);
        await Page.PressAsync("input[id='Search']", "Enter");
        var updatedAssetRow = Page.Locator("tbody tr.data-row:has-text('Manager Brand')").First;
        await updatedAssetRow.Locator("button[title='Return to stock']").ClickAsync();

        // Re-apply search to verify return
        await Page.FillAsync("input[id='Search']", uniqueSerial);
        await Page.PressAsync("input[id='Search']", "Enter");
        var finalAssetRow = Page.Locator("tbody tr.data-row:has-text('Manager Brand')").First;
        ...
        string uniqueRequest = "Req-Manager-" + System.Guid.NewGuid().ToString().Substring(0, 8);
        await Page.FillAsync("input[id='Input_ItemName']", uniqueRequest);
        await Page.SelectOptionAsync("select[id='Input_Category']", new[] { "0" });
        await Page.FillAsync("textarea[id='Input_Reason']", "Manager test request justification.");
        ...
        if (await returnAssetBtn.IsVisibleAsync())
        {
            ...
            await Page.FillAsync("#modal-raiseReturn.open input[name='dropOffLocation']", "Manager Desk");
        ...
        if (await inspectionRow.IsVisibleAsync())
        {
            ...
            await Page.FillAsync("#modal-inspectReturn.open textarea[name='notes']", "IT Manager inspected return.");
        ...
        if (await inProgressRow.IsVisibleAsync())
        {
            ...
            await Page.FillAsync("#modal-resolveService.open textarea[name='resolution']", "IT Manager resolved issue.");
            await Page.ClickAsync("#modal-resolveService.open button[type='submit']:has-text('Mark resolved')");
        }

        // ==========================================
        // PAGE 9: Audit Trail (`/Ops/Audit`)
        // ==========================================
        ...
        await Page.SelectOptionAsync("select[id='RoleName']", new[] { "IT Asset Manager" });
